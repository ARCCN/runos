...
namespace detail {
    ...
    public:
        ...
        {
            ASSERT(m_mask.size() == t.nbits());
            ASSERT(t.maskable() || m_mask.all() || m_mask.none());
        }
    ...
    struct binop_type_func<T, T> {
        constexpr T operator()(const T, const T) const noexcept
        { return T(); }
    };

    template<class T>
    struct binop_type_func<T, type> {
        T operator()(const T lhs, const type rhs) const
        {
            ASSERT(lhs == rhs);
            return T();
        }
    ...
    template<class T1, class T2>
    auto binop_type_impl(const T1 lhs, const T2 rhs)
    {
        return binop_type_func<T1, T2>()(lhs, rhs);
    }

    template<class T1, class T2>
    auto binop_type(const T1 lhs, const T2 rhs)
    {
        return binop_type_impl(lhs.type(), rhs.type());
    }
...
{ return detail::binop_type(lhs, rhs), rhs; }
...
{ return field<T>{detail::binop_type(lhs, rhs), lhs.value_bits(),
                                     lhs.mask_bits() & rhs.mask_bits()}; }
...
{ return field<T>(detail::binop_type(lhs, rhs), lhs.value_bits(), rhs.mask_bits()); }
